       ...
       function dibsCheck(idx) {
    	 ...
    	 function shareMessage() {
		  		 ...
		  		 //값을 받아와야함
					...
					let productNameG = document.getElementById("productNameG").value;
					let detailG = document.getElementById("detailG").value;
					let saleG = document.getElementById("saleG").value;
					let salePriceG = document.getElementById("salePriceG").value;
					let idxG = document.getElementById("idxG").value;
				
					let resultImg = document.querySelector('#resultImg');
					let resultAlt = resultImg.firstElementChild.alt;
		
					const url = 'http://localhost:9090';
					//const url = 'http://49.142.157.251:9090/cjgreen';
					const shareImg = url+'${ctp}/dbShop/product/'+ resultAlt;
					 
					let shareUrl = url+'${ctp}/product/tableWere/dishContent?idx='+idxG;
					if(saleG=='YES'){
						Kakao.Share.createDefaultButton({
			  				  ...
			  				  itemContent: {
			  				    profileText: productNameG,
			  				    profileImageUrl: shareImg,
			  				    titleImageUrl: shareImg,
			  				    titleImageText: productNameG,
			  				    titleImageCategory: '인테리어',
			  				    items: [
				  				      {
				  				        item: '세일가',
				  				        itemOp: salePriceG,
...
<body>
    ...
    <div class="row">
      <div class="col p-3 product-container">
        ...
        <div class="product-image">                       
				 ...
				 <div id="resultImg"> 				  
				  	<img src="${ctp}/dbShop/product/${productVO.FSName}" alt="${productVO.FSName}" />
				 </div>
				</div>
          ...
          <div class="product-options">
            <form name="optionForm"> <!-- Form to display option information -->
              ...
              <label for="quantity">
							...
							<a id="kakaotalk-sharing-btn" href="javascript:shareMessage()" class="fa-regular fa-share-from-square fa-2xl" style="color: #110105;"></a>
            </form>
          </div>
  ...
  <p><br/></p>
  <input type="hidden" id="priceG" value="${productVO.mainPrice}"/> 
  <input type="hidden" id="productNameG" value="${productVO.productName}"/> 
  <input type="hidden" id="detailG" value="${productVO.detail}"/> 
  <input type="hidden" id="saleG" value="${productVO.sale}"/> 
  <input type="hidden" id="salePriceG" value="${productVO.salePrice}"/> 
  <input type="hidden" id="idxG" value="${productVO.idx}"/> 
  <%-- <input type="hidden" id="fSNameG" value="${productVO.fSName}"/> --%> 
