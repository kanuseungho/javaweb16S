	     ...
	     <div class="imageUpdate">
	     	...
	     	<div class="form-group" style="margin-top: 10px;">	     	
	     		<form name="productNameForm" method="post" action="${ctp}/admin/adminProductNameUpdate?idx=${productVO.idx}">
	     			<div class="form-row">
		     			<input type="text" value="${productVO.productName}" name="productNameUpdate" id="productNameUpdate" class="form-control col" />
		     			<div class="col-2">
	     	...
	     	<div class="form-group">
	     		<form name="detailForm" method="post" action="${ctp}/admin/adminDetailUpdate?idx=${productVO.idx}">
	     			<div class="form-row">	     		
	     				<textarea rows="10" cols="30" id="productDetailUpdate" name="productDetailUpdate" class="form-control col">${productVO.detail}</textarea>
	     				<div class="col-2">
	     	...
	     	<div class="form-group">
	     		<form name="salePriceForm" method="post" action="${ctp}/admin/adminSalePriceUpdate?idx=${productVO.idx}">
		     		<div class="form-row">
		     			<input type="text" value="${productVO.salePrice}" id="salePriceUpdate" name="salePriceUpdate" class="form-control col" />
		     			<input type="submit" value="세일가 수정" class="btn btn-danger btn-sm col-4" />
		     		</div>	
	     		</form>
	     	</div>
	     	<div class="form-group">
	     		<form name="mainPriceForm" action="${ctp}/admin/adminMainPriceUpdate?idx=${productVO.idx}" method="post">
		     		<div class="form-row">
	     				<input type="text" value="${productVO.mainPrice}" name="mainPriceUpdate" id="mainPriceUpdate"  class="form-control col" />
	     				<input type="submit" value="메인가 수정" class="btn btn-danger btn-sm col-4" />
     				</div>
	     		</form>
	     	</div>
	     	<div>
	     		<form name="productDelForm" method="post" action="${ctp}/admin/adminProductDelUpdate?idx=${productVO.idx}">
	     			...
	     			<div class="form-row">
		     			<select class="form-control col" name="productDelUpdate" id="productDelUpdate">
		     				<option value="NO" >판매하기</option>
		     				<option value="YES">판매 중지</option>
		     			</select>
			     		<input type="submit" value="판매상태 변경" class="btn btn-danger btn-sm col ml-2" />
		     		</div>
	     		</form>
	     	</div>
	     	<div>
	     		<form name="saleForm" method="post" action="${ctp}/admin/adminSaleUpdate?idx=${productVO.idx}" class="mt-3">		     		
		     			...
		     			<div class="form-row">
		     				<select class="form-control col " name="saleUpdate" id="saleUpdate">
		     					<option value="YES" >세일 하기!</option>
		     					<option value="NO" >세일 중지!</option>
		     				</select>
		     				<input type="submit" value="세일상태 변경" class="btn btn-danger btn-sm col ml-2"/>
		     			</div>
	     		</form>
	     	</div>
	     	<div class="form-group">
	     		<form name="salePercentForm" action="${ctp}/admin/adminSalePercentUpdate?idx=${productVO.idx}" method="post" class="mt-3">
		     		<div class="form-row">	     		
		     			<input type="text" value="${productVO.salePercent}" name="salePercentUpdate" id="salePercentUpdate" class="form-control col" />
		     			<input type="button" onclick="salePercentCheck()" value="세일 퍼센트 수정" class="btn btn-danger btn-sm col" />
